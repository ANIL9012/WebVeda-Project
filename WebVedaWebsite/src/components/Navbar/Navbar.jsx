  ...
  return (
    ...
    >
      <div className="container p-0">
        {/* Logo */}
        <Link className="navbar-brand fw-bold fs-3" to="webveda" onClick={closeNavbar}>
          Web<span className="text-primary">Veda</span>
          <div className="fs-6 fw-normal text-light opacity-75">by Ankur Warikoo</div>
        </Link>

        {/* Hamburger */}
        <button className="navbar-toggler border-0" type="button" onClick={toggleNavbar}>
          {isOpen ? <span className="fs-2 text-white">&times;</span> : <span className="navbar-toggler-icon"></span>}
        </button>

        {/* Menu */}
        <div className={`navbar-collapse ${isOpen ? "show custom-collapse" : ""}`}>
          <ul className="navbar-nav mx-auto fw-normal gap-4 flex-column flex-lg-row align-items-center text-center">
            <li className="nav-item">
              <Link className="nav-link text-white" to="/" onClick={closeNavbar}>Courses</Link>
            </li>
            <li className="nav-item">
              <Link className="nav-link text-white" to="why-webveda" onClick={closeNavbar}>About</Link>
            </li>
            <li className="nav-item">
              <Link className="nav-link text-white" to="testimonials" onClick={closeNavbar}>Students</Link>
            </li>
          </ul>
